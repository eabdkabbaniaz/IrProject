...
with open(r"C:\Users\user\Downloads\IrProject\IrProject\datasets\dataset_antic\stop_words.txt", 'r',encoding='utf-8') as file:
...
def run_search_engine(query, vectorizer_path, tfidf_matrix_path, corpus_path, original_texts_path, top_n=20):
    ...
    df = pd.read_csv(corpus_path, delimiter='\t', header=None, names=['ID', 'Processed_Text'], dtype={'ID': str})

    # df = pd.read_csv(corpus_path, delimiter='\t', header=None, names=['ID', 'Processed_Text'])
    if(original_texts_path==r"C:\Users\user\Downloads\IrProject\IrProject\datasets\dataset_quore\docs.tsv"):
